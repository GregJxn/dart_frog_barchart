...
class ZoneData {
  ZoneData({this.value = 0, this.color});
  int value;
  String? color;
}
...
class BarChartData {
  BarChartData(
      {this.width = 0, this.height = 0, this.zoneCount = 0, this.zones});
  int width = 0;
  int height = 0;
  int zoneCount = 0;
  List<ZoneData>? zones = [];
...
Future<Response> onRequest(RequestContext context) async {
  ...
  if (zoneCount == 0) {
    return Response(statusCode: 422, body: 'No zone data found');
  }
  ...
  for (var i = 0; i < zoneCount; i++) {
    ...
    final zone = ZoneData(
      ...
      color: (palette > 0) ? palettes[palette - 1][i % 5] : zoneColor,
    );
    zoneData.add(zone);
  }
...
Future<img.Image> getLabelledGraphImageBytes(BarChartData graphData) async {
  ...
  final segments = Platform.script.pathSegments.where((s) {
    // filter out bits of the path we dont want
    return !['bin', 'app', '.dart_frog']
        .contains(s); // need to strip these out for running in dev or live
  });
  final pathSegments = segments.take(segments.length - 1);

  var assetsPath = '/fonts';
  if (pathSegments.isNotEmpty) {
    assetsPath = '/' + pathSegments.join('/') + '/runtime';
  }

  var fontFilename = '$assetsPath/Roboto-Medium-32px.ttf.zip';
  var fontHeight = 32;
  ...
  if (requestedWidth <= fontBreakpointWidth ||
    ...
    fontHeight = 16;
    fontFilename = '$assetsPath/Roboto-Medium-16px.ttf.zip';
  ...
  img.fill(image,
      color: img.ColorRgba8(255, 255, 255, 0)); // transparent background
